...
The code lives in its own GitHub repository. A GitHub Actions job (`.github/workflows/update.yml`) runs the updater on GitHub's servers: it refreshes lines, injuries, weather, starters, ratings, the ledger, and the scorecard, then commits the data files back to the repository, and the page reads them straight from GitHub. The clock that starts the job is a Netlify scheduled function (`netlify/functions/kick-updater.mjs`): it wakes every 15 minutes, works out the time in Chicago, and at the wanted looks (10:15 and 17:15 daily, plus game-day looks after the inactive lists on Sunday, Monday, and Thursday) asks GitHub to start the workflow, which begins within seconds. GitHub's own schedule is kept only as a fallback, because it can run hours late. The function needs a `GITHUB_DISPATCH_TOKEN` environment variable on the Netlify site: a fine-grained token for this repository with Actions read and write. Netlify builds the site only when the page itself changes. Nothing depends on a laptop being open. The job can also be started by hand from the repository's Actions tab.
