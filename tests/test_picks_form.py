...
def test_a_pick_stays_out_of_the_public_file_until_its_game_kicks_off():
    kick = {(1, "NE", "SEA"): "2026-09-10T00:20Z", (1, "ATL", "PIT"): "2026-09-13T17:00Z"}
    picks = {"sources": {}}
    subs = [batch("2026-09-08T12:00:00Z", 1, [
        {"away": "NE", "home": "SEA", "winner": "SEA"},
        {"away": "ATL", "home": "PIT", "winner": "PIT"},
    ])]
    before = datetime(2026, 9, 9, 12, tzinfo=timezone.utc)                 # Wednesday noon: nothing has kicked off
    picks, kept = merge_submissions(picks, subs, kick, PEOPLE, before)
    assert kept == 0 and picks["sources"].get("narcisa", {}).get("picks", []) == []
    between = datetime(2026, 9, 10, 12, tzinfo=timezone.utc)               # Thursday: only the opener has
    picks, kept = merge_submissions(picks, subs, kick, PEOPLE, between)
